...
namespace SharpCompress.Compressors.Deflate64
{
	...
	public sealed class Deflate64Stream : Stream
    {
        ...
        public Deflate64Stream(Stream stream, CompressionMode mode)
        {
            ...
            InitializeInflater(stream, CompressionMethod.Deflate64);
        }

        /// <summary>
        /// Sets up this DeflateManagedStream to be used for Inflation/Decompression
        /// </summary>
        private void InitializeInflater(Stream stream, CompressionMethod method = CompressionMethod.Deflated)
        {
            Debug.Assert(stream != null);
            Debug.Assert(method == CompressionMethod.Deflated || method == CompressionMethod.Deflate64);
            if (!stream.CanRead)
                throw new ArgumentException("Deflate64: input stream is not readable", nameof(stream));

            _inflater = new InflaterManaged(method == CompressionMethod.Deflate64);
